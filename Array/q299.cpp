...
namespace std
{
	...
	public:
	    string getHint(string secret, string guess) {
	        int len=secret.size();
	        vector<int> sect(10,0);
	        vector<int> gues(10,0);
	        int ib=0,ic=0;
	        for(int i=0;i<len;i++)
	        {
	            if (secret[i]==guess[i]) ib++;
	            else
	            {
	                sect[secret[i]-'0']++;
	                gues[guess[i]-'0']++;
	        ...
	        for(int i=0;i<10;i++)
	        {
	            if (sect[i]>0) ic+=((gues[i]>sect[i])?sect[i]:gues[i]);
	        }
	        
	        string rst=to_string(ib)+'A'+to_string(ic)+'B';
	        return rst;
	    }
	};
...
/****************************************************************************************************
                                             Note
understand the question. it could be some catches. 
****************************************************************************************************/