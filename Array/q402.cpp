...
namespace std
{
	...
	public:
	    string removeKdigits(string num, int k) {
	        ...
	        for(int i=0;i<len;i++)
	        {
	            while(rst.size()>0 && num[i]<rst.back() && k>0)
	            {
	                rst.pop_back();
	                k--;
	            }
	        ...
	        rst=rst.substr(0,keep);
	        int s=0;
	        while(s<rst.size() && rst[s]=='0') s++;
...
/****************************************************************************************************
                                             Note
Initially, I think it is dp problem. And indeed it can be solved by dp. 
However, it is acturally the greedy. becasue it has to put the smallest to the top as possible. 
****************************************************************************************************/